...
def load_reinforce_agent(checkpoint_path):
    env = Quatro()
...
def evaluate_vs_random(agent, env_class, num_eval_episodes):
    ...
    for episode_idx in range(num_eval_episodes):
        ...
        # On alterne le joueur qui commence pour éviter un biais
        ...
        while not env.is_terminal():
            ...
            # Une victoire est déclenchée uniquement après une action de pose
            if reward == 1.0:
...
def main():
    ...
    try:
        for milestone in CONFIG["eval_milestones"]:
            ...
            agent = load_reinforce_agent(checkpoint_path)
